...
MODEL_PATH = "models/cogcess_lexical_model.pth"
...
print("After removing duplicate words:", len(df))


# ============================================================
# CREATE BINARY DIFFICULTY LABEL
# ============================================================
...
y_class = df["is_difficult"].values.astype(np.float32)

y_reg = df["lexical_difficulty"].values.astype(np.float32)
...
class CogcessLexicalClassifier(nn.Module):
...
class CogcessLexicalRegressor(nn.Module):
...
classifier = CogcessLexicalClassifier(
    X.shape[1]
)
...
# Test only genuinely difficult words
test_difficult_mask = (
    y_class_test == 1
)
...
regressor = CogcessLexicalRegressor(
    X.shape[1]
)
...
for i in range(sample_count):
    ...
    print(
        f"Actual: {y_reg_test_filtered[i]:6.2f} | "
        f"Predicted: {reg_predictions[i]:6.2f}"
    )
